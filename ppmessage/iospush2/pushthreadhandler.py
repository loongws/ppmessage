...
class PushThreadHandler():
    ...
    def __init__(self, _app):
        ...
        self.language = _get_config.get("server").get("language").get("locale") or "en_US"
    ...
    def _push(self, _apns, _body, _user_config):
        ...
        _title = push_title(_body.get("mt"), _body.get("ms"), _body.get("bo"), self.language)
        ...
        _client = APNsClient(_apms.get("combination_pem"), use_sandbox=False, use_alternative_port=False)
        if _dev != None and _dev == True:
            _client = APNsClient(_apms.get("combination_pem"), use_sandbox=True, use_alternative_port=False)
